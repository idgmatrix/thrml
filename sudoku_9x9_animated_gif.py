...
# Simulated annealing to solve Sudoku with frame capture for animation
def solve_sudoku_with_animation():
    # Initialize grid
    ...
    frame_titles.append("Initial Sudoku")
    frame_violations.append(current_violations)
    ...
    for iteration in range(max_iterations):
        ...
        if current_violations == 0 and is_valid_solution(current_grid):
            ...
            frames.append(current_grid.copy())
            frame_titles.append("Solution Found!")
            frame_violations.append(0)
            frame_temperatures.append(0)
            return frames, frame_titles, frame_violations, frame_temperatures, iteration
        ...
        # Store frame every N iterations
        if iteration % 100 == 0:
            frames.append(current_grid.copy())
            frame_titles.append(f"Iteration {iteration}")
            frame_violations.append(current_violations)
            frame_temperatures.append(temperature)
    ...
    # Store final frame
    frames.append(best_grid.copy())
    frame_titles.append("Final State")
    frame_violations.append(best_violations)
    frame_temperatures.append(temperature)
...
# Main execution
print("Solving Sudoku puzzle and creating animation...")

# Solve the puzzle and get frames
frames, frame_titles, frame_violations, frame_temperatures, iterations = solve_sudoku_with_animation()

print(f"Animation will have {len(frames)} frames")

# Create directory for frames
import os
if not os.path.exists('sudoku_frames'):
    os.makedirs('sudoku_frames')

# Save individual frames as PNG
print("Saving animation frames...")
for i, (frame, title, violations, temperature) in enumerate(zip(frames, frame_titles, frame_violations, frame_temperatures)):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 7))

    # Draw Sudoku grid
    draw_sudoku_grid(ax1, frame, title)

    # Draw violations/temperature plot
    ax2_twin = ax2.twinx()
    iterations_so_far = list(range(i + 1))
    violations_so_far = frame_violations[:i + 1]
    temperatures_so_far = frame_temperatures[:i + 1]

    line1, = ax2.plot(iterations_so_far, violations_so_far, 'b-', label='Violations', linewidth=2)
    line2, = ax2_twin.plot(iterations_so_far, temperatures_so_far, 'r-', label='Temperature', linewidth=2)
    ...
    # Save frame
    filename = f'sudoku_frames/frame_{i:04d}.png'
    fig.savefig(filename, dpi=100, bbox_inches='tight')
    plt.close(fig)

    if i % 20 == 0:
        print(f"Saved frame {i}/{len(frames)-1}")

print("All frames saved. Creating GIF...")
...
try:
    ...
    # Get all frame files
    frame_files = sorted(glob.glob('sudoku_frames/frame_*.png'))

    if frame_files:
        # Open all frames
        frames_pil = []
        for filename in frame_files:
            frames_pil.append(Image.open(filename))

        # Save as GIF
        frames_pil[0].save('sudoku_solving_animation.gif',
                          save_all=True,
                          append_images=frames_pil[1:],
                          duration=50,  # 50ms per frame
                          loop=0)  # Loop forever

        print("GIF saved successfully as 'sudoku_solving_animation.gif'")
    else:
        print("No frame files found")

except ImportError:
    print("PIL not available, GIF not created. Individual frames saved in 'sudoku_frames' folder.")
except Exception as e:
    print(f"Error creating GIF: {e}. Individual frames saved in 'sudoku_frames' folder.")

# Also save the final solution
solution_grid = frames[-1]
print(f"\nFinal solution grid:")
print(solution_grid)
